...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const defaultCookieSideBarValue = getCookieValue(`${SIDEBAR_COOKIE_NAME}`)
  
  // get browser dark mode preference
  const defaultCookieThemeValue = getCookieValue(`${THEME_COOKIE_NAME}`)
  // const browserDarkmodeDefaultPreference = getBrowserDarkModeThemePreference()
  
  const convertCookieValToBoolean =(val: string | null)=> {
    if(!val) return true
    switch (val){
  ...
  const convertDefaultCookieThemeValue = (val: string | null)=> {
    if(!val) return false
    switch (val){
  ...
  const [isDark, setIsDark] = useState(convertDefaultCookieThemeValue(defaultCookieThemeValue));
  
  // This is the internal state of the sidebar.
  // We use openProp and setOpenProp for control from outside the component.
  const [_open, _setOpen] = useState(convertCookieValToBoolean(defaultCookieSideBarValue));
  ...
  // set the dark mode style to the document body on component mount
  useEffect(()=> {
  if(defaultCookieThemeValue && defaultCookieThemeValue === "dark"){
  document.documentElement.classList.add("dark");
  }
  },[defaultCookieThemeValue])
  ...
  const contextValue = useMemo<SidebarContext>(
    ...
    [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, isDark],
  );
...
const SidebarTrigger1 = forwardRef<React.ElementRef<typeof Button>, React.ComponentProps<typeof Button>>(
  ({ className, onClick, ...props }, ref) => {
...
SidebarTrigger.displayName = "SidebarTrigger1";
  ...
  ({ className, ...props }, ref) => (
    <ul
      ...
      className={cn(
        ...
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-2 border-l border-sidebar-border py-0.5 pt-2.5",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
